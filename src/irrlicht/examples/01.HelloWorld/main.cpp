...
int main()
{
	...
	*/
	IAnimatedMesh* mesh = smgr->getMesh("../../../../bin/models/mushroom.dae");
	if (!mesh)
	{
		device->drop();
		return 1;
	...
	*/
	if (node)
	{
		...
		node->setMaterialTexture( 0, driver->getTexture("../../../../bin/textures/mushroom/mushroom.png") );
	...
	*/
	smgr->addCameraSceneNode(0, vector3df(-40,20,-40), vector3df(0,5,0));
...
RealisticWaterSceneNode*	water =new RealisticWaterSceneNode(smgr, 512,512,driver->getTexture("data/waterbump.png"),dimension2du(512, 512), smgr->getRootSceneNode(), 666);
